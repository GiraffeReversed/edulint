...
def test_local_custom(lines: List[str], expected_output: List[Problem]) -> None:
    create_apply_and_lint(
        lines,
        [Arg(Option.PYLINT, "--enable=local-defects")],
        expected_output
    )
...
def test_no_is_custom(lines: List[str], expected_output: List[Problem]) -> None:
    create_apply_and_lint(
        lines,
...
def test_ord_files(filename: str, expected_output: List[Problem]) -> None:
    apply_and_lint(
        filename,
        [
...
def test_local_files(filename: str, expected_output: List[Problem]) -> None:
    apply_and_lint(
        filename,
        [Arg(Option.PYLINT, "--enable=local-defects")],
        expected_output
